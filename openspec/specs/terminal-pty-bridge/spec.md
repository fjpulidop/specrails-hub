...
Clients SHALL send resize control messages `{"type":"resize","cols":N,"rows":M}` when the xterm container resizes. The server MUST call `pty.resize(cols, rows)` and update the session's stored dimensions. If multiple clients are attached with different viewport sizes, the server MUST honor the most recent resize.
...
#### Scenario: Resize debounced on drag
- **WHEN** the user drags the panel height continuously triggering 50 resize events in 200ms
- **THEN** no more than one resize message is sent every 50ms (client-side throttle)
