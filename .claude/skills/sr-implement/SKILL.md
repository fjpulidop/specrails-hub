...
Each architect scaffolds the OpenSpec artifacts in `openspec/changes/<name>/` **using the openspec CLI (the library)** — `openspec new change` + `openspec instructions <artifact> --change <name>` — NOT by hand-rolling the structure or a custom tasks format (see the architect agent's "Step 0"). The CLI is non-interactive and cross-platform (it is a Node binary on PATH; do not assume bash). `tasks.md` MUST use the openspec checkbox template (`## N. group` + `- [ ] N.M …`) so the developer/reviewer can mark `- [x]` and `openspec archive` works.

Each agent's prompt should include:
- Description of the feature
- Context from exploration (if applicable)
- Instruction to scaffold via the openspec CLI and author proposal.md, specs (delta), design.md, and tasks.md through `openspec instructions`, plus the sr-specific `context-bundle.md` (the only artifact not part of the openspec scaffold)
- Per-task metadata REQUIRED on every checkbox line (so this orchestrator's shared-file / layer analysis in 3a.1 still works): a layer tag in the description (`- [ ] N.M [layer] …`) and a `**Files:**` sub-bullet listing `Create:`/`Modify:` paths
