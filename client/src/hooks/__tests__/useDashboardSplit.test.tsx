...
describe('useDashboardSplit', () => {
  ...
  it('initialises at the 65/35 default split when no stored value exists', () => {
    // viewport 1400 → round(1400 * 0.65) = 910
    const { result } = renderHook(() => useDashboardSplit('proj-1'))
    expect(result.current.leftWidth).toBe(910)
  ...
  it('canonical default clamps to viewport - MIN_RIGHT_PX on small viewports', () => {
    setViewport(1000)
    const { result } = renderHook(() => useDashboardSplit('proj-1'))
    // round(1000 * 0.65) = 650; clamp(650) within [320, 720] = 650.
    expect(result.current.leftWidth).toBe(650)
  })
  ...
  it('resetToDefault restores the canonical default and persists', () => {
    ...
    expect(result.current.leftWidth).toBe(910)
    expect(localStorage.getItem('specrails-desktop:dashboard-split:proj-1')).toBe('910')
  })
