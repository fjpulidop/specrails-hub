...
**Entry point: read the OpenSpec library's apply instructions first.** Before writing any files, get the schema-aware apply guidance from the openspec CLI (the library — non-interactive and cross-platform; it is a Node binary on PATH, so it runs identically on macOS/Linux/Windows — do NOT call the interactive `opsx:*` skills and do NOT use bash-only syntax):

```
openspec instructions apply --change "<specName>"
```

This returns the ordered task checklist and the apply protocol. Work the tasks **in order**, and as you finish each one, flip its checkbox in `openspec/changes/<specName>/tasks.md` from `- [ ]` to `- [x]` (use the Edit tool). Do NOT batch the checkbox updates at the end — mark each task complete the moment it is done, so progress is accurate if the run is interrupted. Do NOT write production files without first reading the apply instructions.

**After the apply cycle — Checkbox Verification Gate:**
