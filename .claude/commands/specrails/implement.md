...
| Tool | Status | Notes |
|------|--------|-------|
| Backlog provider | ok/missing | Local Tickets |
| OpenSpec | ok | ... |
| Dependencies | ok | ... |
| Test runner | ok | ... |
```
...
- Tags for each task: [backend], [frontend], [cli]
...
- If ALL tasks are tagged `[backend]` or `[cli]` only → use `sr-backend-developer`
- If ALL tasks are tagged `[frontend]` only → use `sr-frontend-developer`
- If tasks span both `[backend]` and `[frontend]` → use `sr-developer` (full-stack)
- If unsure → default to `sr-developer`
...
5. Create PR (if GitHub CLI is available):
   ```bash
   gh pr create --title "<feature-title>" --body "<summary of changes>" --base main
   ```
...
- For fully resolved issues/tickets: add a comment noting completion and reference the PR:
  ```bash
  Write to `.specrails/local-tickets.json` using the advisory locking protocol: acquire lock → read file → append comment to `tickets["{id}"].comments`, set `updated_at` to now, bump `revision`, update `last_updated` → write → release lock.
  ```
  - **Local:** Update the ticket status to `"done"` using `Write to `.specrails/local-tickets.json` using the advisory locking protocol: acquire lock → read file → update fields in `tickets["{id}"]`, set `updated_at` to now, bump `revision`, update `last_updated` → write → release lock.` and add a comment: `"Implemented in PR #XX. All acceptance criteria met."` via `Write to `.specrails/local-tickets.json` using the advisory locking protocol: acquire lock → read file → append comment to `tickets["{id}"].comments`, set `updated_at` to now, bump `revision`, update `last_updated` → write → release lock.`. Local tickets are closed directly — there is no auto-close-on-merge mechanism.
...
- For partially resolved issues/tickets: add a comment noting progress:
  ```bash
  Same as BACKLOG_COMMENT_CMD but append a progress comment with `"type": "progress"`.
  ```
  - **Local:** Additionally update the ticket status to `"in_progress"` via `Write to `.specrails/local-tickets.json` using the advisory locking protocol: acquire lock → read file → update fields in `tickets["{id}"]`, set `updated_at` to now, bump `revision`, update `last_updated` → write → release lock.` if it is still `"todo"`.
