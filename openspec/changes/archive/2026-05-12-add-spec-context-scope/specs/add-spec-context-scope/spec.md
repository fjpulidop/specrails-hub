...
On first open in a project (no persisted value), the toggles SHALL boot to: `specrails specs = ON`, `openspec specs = OFF`, `Full codebase = OFF in Quick / ON in Explore`, `External tools (MCPs) = value of the project's explore_mcp_enabled global setting`.

#### Scenario: Fresh project Quick boot
- **WHEN** the modal opens in Quick mode for a project with no persisted scope and global `explore_mcp_enabled=false`
- **THEN** `specrails specs` is ON
- **AND** `openspec specs` is OFF
- **AND** `Full codebase` is OFF
- **AND** `External tools (MCPs)` is OFF and disabled

#### Scenario: Fresh project Explore boot with global MCP enabled
- **WHEN** the modal opens in Explore mode for a project with no persisted scope and global `explore_mcp_enabled=true`
- **THEN** `specrails specs` is ON
- **AND** `openspec specs` is OFF
- **AND** `Full codebase` is ON
- **AND** `External tools (MCPs)` is ON
