...
### Ask the Hub (Cmd+K project memory)

A global Cmd+K modal that searches and answers natural-language questions about the active project using tickets, Explore conversations, jobs, file summaries, and recent git commits. Gated by `SPECRAILS_ASK_HUB` (default on — set to `0|false|off` to disable wholesale; router returns 404 and client modal unmounts).

**Server (`server/ask/`)**: `embedder.ts` lazy-loads the bundled `multilingual-e5-small` ONNX model via `@xenova/transformers` (runs in-process, prefetched 5 s post-boot from `server/index.ts`). Missing model files → deterministic hash-based fallback so search-only mode still functions. `chunker.ts` produces `AskDoc` units per kind (ticket / explore-turn / job / file-summary / git-commit). `storage.ts` writes to migration 24's tables: `ask_docs` (BLOB embedding + metadata), `ask_docs_fts` (FTS5 sidecar with INSERT/UPDATE/DELETE triggers), `ask_query_log` (queries + thumbs feedback). `indexer.ts::upsertDoc` invalidates by `body_hash` — unchanged hash skips the embed call. `enumerator.ts` enumerates all sources for backfill; `backfill.ts` runs the one-shot pipeline with WS progress events. `search.ts` runs hybrid BM25 + cosine + Reciprocal Rank Fusion (k=60), reranked by a recency × kindWeight × fusedScore heuristic (or by an LLM rerank when `hub_settings.ask_reranker='llm'`). FIFO cap at 100 000 docs/project; non-ticket docs swept oldest-first when exceeded.

**Intent router + pipelines (`server/ask/intent-router.ts` + `pipelines/`)**: heuristic regex classifies every query into `factual` / `status` / `compare` / `decision`. `factual` and `decision` use retrieval; `decision` boosts `explore-turn` and `git-commit` kinds. `status` bypasses retrieval entirely and aggregates SQL over tickets / jobs / `ai_invocations` (via `getSpending`) / `file_provenance` / `job_profiles` to produce a sub-2000-token context. `compare` runs a GROUP BY over `ai_invocations` by model.

**Answer LLM (`server/ask/spawn-one-shot.ts` + `answer.ts`)**: provider-agnostic via the existing `ProviderAdapter` contract — a new `SpawnAction='ask-answer'` is added to `server/providers/types.ts` and implemented in both `claude-adapter.ts` (`--max-turns 1 --system-prompt -p`) and `codex-adapter.ts` (folded system+user with `exec --json`). The answer endpoint spawns the resolved provider via `spawn-one-shot.ts` and parses the structured JSON envelope `{answer, citations, followups}`. `extractEnvelope` is tolerant of code-fence wrappers; `stripUnresolvedCitations` removes `[N]` markers outside the source range. Every answer writes one `ai_invocations` row with `surface='ask'` (success / aborted / failed) via `finaliseInvocationResult`, surfacing cost on `/analytics`.

**Provider selection (`server/ask/provider-detect.ts`)**: `detectAvailableProviders()` runs each registered adapter's `detectInstalled()` in parallel. `resolveAskProvider(setting, detected)` is the state machine — `none`, `use`, `degraded` (configured provider not on PATH), or `first-run` (≥2 usable and no setting). Per-project override is intentionally not exposed in v1; the answer LLM only synthesises sources so provider affinity with the project is irrelevant.

**REST (`server/ask-router.ts`)** mounted under `/api/projects/:projectId/ask/*`, gated by `isAskHubEnabled()` (404 when off): `GET /search` (instant BM25, < 50 ms p95), `POST /query` (SSE: `sources → token* → citation → followups → invocation → done`, supports client-side AbortController), `GET /index/status`, `POST /index/rebuild`, `GET /history`, `DELETE /history`, `POST /history/:id/rating`, `GET /providers`. Budget cap: monthly `surface='ask'` total checked pre-flight, emits `event: error reason='budget'` when met; search endpoint unaffected.

**Hub settings (`GET/PATCH /api/hub/ask-settings`)**: `ask_answer_provider` (`claude|codex|none|null`), `ask_answer_model_claude` (default `claude-haiku-4-5`), `ask_answer_model_codex` (`gpt-4o-mini`), `ask_reranker` (`llm|heuristic|none`), `ask_auto_index_on_first_open` (boolean), `ask_hotkey`, `ask_monthly_budget_usd` (default `5`). Reachable from `Settings → Ask the Hub` in `GlobalSettingsPage`.

**Client**: `client/src/components/ask/AskHubProvider.tsx` mounts at App root inside `HubProvider`; registers a global Cmd+K listener that is suppressed while another `[role="dialog"]` is active. `AskHubModal.tsx` (portal to body) shows instant grouped search results, a first-run `FirstRunProviderPicker` when ≥2 providers detected and no setting, a streaming `AnswerView` that renders inline `[N]` citation chips via `CitationChip.tsx` (kind-aware click dispatch — ticket → `TicketDetailModalProvider`, job → navigate, explore-turn → navigate + scroll, file-summary → `/code?path=`, git-commit → SHA-to-clipboard), follow-up chips, and 👍/👎 thumbs feedback. Per-project history in `localStorage['specrails-hub:ask-history:<projectId>']`, capped at 20.

**WS events** (project-scoped via `boundBroadcast`): `ask.indexing { phase, current, total }`, `ask.index_updated { added, updated, removed }`, `ask.degraded { reason }`.

**Event-driven incremental indexing**: `QueueManager` post-exit re-indexes touched tickets + the job doc. `ChatManager` debounces explore-turn re-indexes 5 s on every persisted assistant message. `FileSummaryManager` re-indexes summary docs on update. All hooks are fire-and-forget under `isAskHubEnabled()` — failures never block the rail / chat / summary path.

**Bundled artifact**: `src-tauri/binaries/embeddings/{model.onnx, tokenizer.json, config.json}` (~118 MB quantized e5-small). Tracked via Git LFS (`.gitattributes`). Bundled into the desktop installer by `scripts/build-sidecar.mjs`. The npm package's `files` whitelist excludes the directory. CI for unit tests does not need LFS — the embedder is mocked at the module boundary.

**Reserved paths (hub-managed)**: `~/.specrails/projects/<slug>/jobs.sqlite` (migration 24 adds `ask_docs*` and `ask_query_log`). Nothing new in the user's working tree.

**ai_invocations surface**: `'ask'` is now a recognised value in the allow-list (`server/ai-invocations.ts`), in the `Surface` union type (server + client `client/src/types/spending.ts`), in `getSpending` aggregation, and in the client's `SURFACE_LABEL` / `SURFACE_ACCENT` maps (mapped to `accent-primary`).

**v1 limitations**: worker_thread embedder (the file exists at `server/ask/embedder-worker.ts` but the main thread uses the inline path); reranker LLM path (heuristic is the working default); deep-ask agentic tools; cross-project search; voice input. The 3 historical one-shot spawn sites (`contract-refine-runner`, `tickets/generate-spec`, `agent-refine-manager`) are intentionally NOT refactored to the new `ask-answer` action in this change — they have their own argv shapes; the spec marks the refactor as deferred.

**Rollback**: `SPECRAILS_ASK_HUB=0`. `ask_docs*` rows remain on disk (additive migration); next deploy reads them as orphans and is unaffected.
