...
6. **Archive** — Only reachable when Step 5 gate passes. Use the **openspec CLI** (the library — non-interactive and cross-platform; it is a Node binary on PATH, so it runs identically on macOS/Linux/Windows. Do NOT call the interactive `opsx:*` skills; do NOT use bash-only syntax). Validate, then archive:
   ```
   openspec validate "<specName>" --strict
   openspec archive "<specName>" --yes
   ```
   `openspec archive` moves the change to `openspec/changes/archive/` and syncs its delta specs into the main specs under `openspec/specs/`. Run each command on its own; do NOT prompt the user or ask for confirmation.
